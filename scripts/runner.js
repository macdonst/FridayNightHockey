...
module.exports = async (req, res, next) => {
  ...
  let action = actionName === 'html' ? null : serverlessConfig['functions'][actionName]
  if (!action) {
    return res.status(404)
      .send({ error: '404: Action ' + actionName + ' not found' })
  }

  const handlerPath = action.handler.replace('.main', '.js')

  const handlerFunction = require(path.join(__dirname, '../', handlerPath))

  try {
    let response = await handlerFunction.main(params)
